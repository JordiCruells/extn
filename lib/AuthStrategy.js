...
var util = require('util')
  ...
  , parse = require('../viewModels/user').parse
  ;
...
function Strategy(options, verify) {
  ...
  console.log('1-function Strategy(options, verify)');

  options = options || {};

  options.authorizationURL = options.authorizationURL || options.authorizationUrl;

  options.tokenURL = options.tokenURL || options.tokenUrl;

  console.log('options.authorizationURL: ' + options.authorizationURL);
  ...
  console.log('options.profileURLL: ' + options.profileURL);
  me.profileUrl = options.profileURL;
  me.name = 'oauth2-strategy';
}
...
Strategy.prototype.userProfile = function (accessToken, done) {
  ...
  console.log('Strategy.prototype.userProfile');
  console.log('acces token:' + accessToken);

    //done(null, {id:1, name:'jordi'});
  ...
  var me = this;
    ...
    me._oauth2.get(
      ...
      , function (err, body/*, res*/) {
          ...
          console.log('ERROR WHEN GETTING PROFILE: ' + JSON.stringify(err));

          if (err) { return done(new InternalOAuthError('failed to fetch user profile', err)); }

          console.log('body is: ' + body);
          console.log('JSON: ' + JSON);
          console.log('JSON.parseprofile: ' + JSON.parseProfile);
      ...
      profile._json = json;
            ...
            console.log('done(null, profile): ' + JSON.stringify(profile));
            done(null, profile);
    }
  );
};
